...
export default function Strip({ id, breakdown, elements, onSelect: _onSelect, onContextMenu }: StripProps) {
  ...
  const { bg, text: textColor } = getStripColors(breakdown.intExt, breakdown.dayNight);

  const castElements = elements.filter(e =>
    breakdown.elements.includes(e.id) && e.category === 'Cast'
  );
  ...
  return (
    ...
    >
      ...
      {/* Scene number */}
      <div className="w-10 flex-shrink-0 px-1 font-bold text-center truncate">
        {breakdown.sceneNumber}
      </div>

      {/* INT/EXT badge */}
      <div className="w-16 flex-shrink-0 px-1 text-center">
        <span className="font-medium">{breakdown.intExt}</span>
      </div>

      {/* D/N badge */}
      <div className="w-12 flex-shrink-0 px-1 text-center font-medium">
        {breakdown.dayNight}
      </div>
      ...
      {/* Page count */}
      <div className="w-12 flex-shrink-0 px-1 text-center text-xs font-mono">
        {breakdown.pageCount}
      </div>
      ...
      <div className="w-20 flex-shrink-0 px-1 flex gap-0.5 flex-wrap overflow-hidden">
        {castElements.slice(0, 5).map(el => {
          const num = el.name.match(/#(\d+)/)?.[1] || el.name.match(/\((\d+)\)/)?.[1] || '';
          return (
            <span
              key={el.id}
              className="text-xs px-1 rounded font-bold"
              style={{ backgroundColor: '#374151', color: '#f3f4f6', fontSize: '9px' }}
              title={el.name}
            >
              {num}
            </span>
